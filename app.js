...
es:{title:"APRENDIZAJE DE PROTECCIÓN",none:"Todavía no hay actividad protegida registrada.",summary:(sessions,attempts,success,reduction)=>`${sessions} sesiones protegidas y ${attempts} intentos protegidos registrados. El ${success}% de los intentos redujo la intensidad, con una reducción general del ${reduction}%.`,category:(name,count)=>`Categoría protegida más frecuente: ${name} (${count} intento${count===1?"":"s"}).`,domain:(name,count)=>`Dominio protegido más frecuente: ${name} (${count} intento${count===1?"":"s"}).`,mode:(name,count)=>`Modo Protection más utilizado: ${name} (${count} intento${count===1?"":"s"}).`,events:count=>`${count} eventos de Protection registrados.`},
it:{title:"APPRENDIMENTO DELLA PROTEZIONE",none:"Non è ancora stata registrata alcuna attività protetta.",summary:(sessions,attempts,success,reduction)=>`${sessions} sessioni protette e ${attempts} tentativi protetti registrati. Il ${success}% dei tentativi ha ridotto l'intensità, con una riduzione complessiva del ${reduction}%.`,category:(name,count)=>`Categoria protetta più frequente: ${name} (${count} tentativ${count===1?"o":"i"}).`,domain:(name,count)=>`Dominio protetto più frequente: ${name} (${count} tentativ${count===1?"o":"i"}).`,mode:(name,count)=>`Modalità Protection più utilizzata: ${name} (${count} tentativ${count===1?"o":"i"}).`,events:count=>`${count} eventi Protection registrati.`}
};
...
if(!sessions.length){const empty=document.createElement("p");empty.textContent=t("insights.empty",copy.none);recent.appendChild(empty);renderProtectionInsights(recent);return}
...
recent.appendChild(facts);renderProtectionInsights(recent);
