...
document
.querySelectorAll(”.choice-button.selected”)
.forEach(button=>button.classList.remove(“selected”));

document
    .querySelectorAll(".outcome-button.selected")
    .forEach(button=>button.classList.remove("selected"));
...
document
.querySelectorAll(”#behaviorOptions .choice-button”)
...
const value=
$(“otherBehaviorInput”).value.trim();

if(session.behavior==="other"){
    session.behaviorLabel=value;
    $("behaviorContinueButton").disabled=
        value.length===0;
}
...
document
.querySelectorAll(”#expectationOptions .choice-button”)
.forEach(item=>item.classList.remove(“selected”));
...
const value=
$(“otherExpectationInput”).value.trim();

if(session.expectation==="other"){
    session.expectationLabel=value;
    $("expectationContinueButton").disabled=
        value.length===0;
}
...
getSavedSessions().forEach(item=>{
    if(!item)return;
    if(
        Array.isArray(item.attemptHistory)&&
...
return INTERRUPT_ADAPTIVE.buildStats(
getRelevantHistory()
);
}

function getAdaptiveStats(){
return INTERRUPT_ADAPTIVE.getAdaptiveStats(
getEngineHistory(),
session
);
}
...
return INTERRUPT_ADAPTIVE.getLastSuccessful(
getEngineHistory(),
session,
intervention
);
...
return INTERRUPT_ADAPTIVE.getRecommendation(
getEngineHistory(),
session,
intervention
);
...
const attempted=
new Set(session.interventionAttempts);
...
candidates.sort(
    (a,b)=>
        INTERRUPT_ADAPTIVE.scoreIntervention(
            b,
            stats,
            preferred,
...
getRecommendation(session.intervention);

if(!recommendation)return;
const box=document.createElement("div");
box.className="insight-section";
const before=
    recommendation.last.intensityBefore;
const after=
    recommendation.last.intensityAfter;
const confidence=
...
if(recommendation.reduction>0){
    if(recommendation.uses===1){
        text=
            `Last time, this type of urge dropped from ${before} to ${after} with ${interventionTitle(session.intervention)}.`;
    }else if(confidence==="high"){
        text=
            `${interventionTitle(session.intervention)} has repeatedly reduced this type of urge for you.`;
    }else{
        text=
            `You have reduced a similar urge with ${interventionTitle(session.intervention)} before.`;
    }
}else{
    text=
        `You tried ${interventionTitle(session.intervention)} before, but the urge did not decrease.`;
}
...
Number(
session.attemptIntensityBefore??
session.intensityBefore
);
...
content.appendChild(box);
const timerBox=
    box.querySelector(".wave-timer");
const timerValue=
    box.querySelector(".timer-value");
box
...
finish
    .querySelectorAll("[data-wave-result]")
    .forEach(button=>{
        button.addEventListener(
            "click",
            ()=>{
...
box.innerHTML=`
    ...
    <div class="option-grid">
        ${Object.keys(
            t("interventions.actualNeed.options",{})
        ).map(key=>`
...
content.appendChild(box);
const suggestion=
    box.querySelector(".need-suggestion");
box
        ...
        button.addEventListener(
            ...
            ()=>{
                ...
                suggestion.textContent=
                    t(
                        `interventions.actualNeed.suggestions.${key}`
                    );
                actions.innerHTML="";
                addActionButton(
                    actions,
...
box.innerHTML=`
    ...
    <div class="option-grid">
        ${Object.keys(
            t("interventions.breakChain.options",{})
        ).map(key=>`
...
content.appendChild(box);
const result=
    box.querySelector(".trigger-result");
box
    ...
    .forEach(button=>{
        button.addEventListener(
            ...
            ()=>{
                ...
                result.textContent=
                    t(
                        "interventions.breakChain.result"
                    );
                actions.innerHTML="";
                addActionButton(
                    actions,
                    t(
                        "interventions.breakChain.changeScene"
                    ),
                    finishIntervention
                );
            }
...
content.appendChild(box);
const timer=
    box.querySelector(".switch-timer");
addActionButton(
    actions,
...
content.appendChild(box);
const range=
    box.querySelector(".intervention-range");
const value=
    box.querySelector(".certainty-value");
...
session.attemptStartedAt=
    new Date().toISOString();
if(
    !session.interventionAttempts.includes(
        intervention
    )
){
    session.interventionAttempts.push(
        intervention
    );
}
$("interventionCategory").textContent=
...
const now=
    new Date().toISOString();
if(!Array.isArray(session.attemptHistory)){
...
document
.querySelectorAll(”.outcome-button”)
.forEach(item=>
item.classList.remove(“selected”)
);
...
function showResult(){
const before=
Number(session.intensityBefore);

const after=
    Number(session.intensityAfter);
...
entries.forEach(([id,data])=>{
    if(
        !best||
        data.averageImpact>best.average
    ){
        best={
            id,
...
Object.entries(triggerCounts).forEach(
    ([trigger,count])=>{
        if(
            !commonTrigger||
            count>commonTrigger.count
        ){
            commonTrigger={
                trigger,
                count
    ...
    .forEach(item=>{
        ...
        row.innerHTML=`
            ...
            <div class="recent-session-change">
                ${item.intensityBefore} → ${item.intensityAfter} (${change})
            </div>
        `;
...
$("insightsDoneButton").addEventListener(
    "click",
    ()=>showScreen("home")
);
document
...
document
    .querySelectorAll(
        "#expectationOptions .choice-button"
    )
    .forEach(button=>
        button.addEventListener(
            "click",
...
getRecommendation,
band:value=>
INTERRUPT_ADAPTIVE.getIntensityBand(
value
),
resetData:()=>{
...
STORAGE.sessions
        ...
        console.log(
            "INTERRUPT session data cleared."
        );
    },
